...
Before the supervisor spawns a wave or a recursive child, **all six checks must
pass**, in order:
...
| 2 | **Budget check** | Live counters for waves, child agents, wall-clock, tool calls, model calls, tokens, **and `evidence_fetch_budget` (Knowledge Layer reads/refetches/stale-acceptance)**. | Block; emit Decision log entry `spawn_denied: budget_exhausted`; escalate via §6 if `escalate_on_exhaustion`. |
...
| 4 | **Evidence sufficiency** | The proposed child has, in scope, a `claim_evidence_map_ref` reachable through the Knowledge Layer, or a declared intent to fetch one through a tool-plan-permitted `read_*` tool. A spawn that would write to a system of record without any path to grounded evidence is **never** approved. See [knowledge-layer.md](knowledge-layer.md). | Block; emit Decision log entry `spawn_denied: evidence_shortfall`; escalate to HITL per §6 (**Evidence shortfall** trigger). |
| 5 | **Provenance log** | Spawn ledger row written before the child is dispatched. | If the ledger write fails, the spawn is aborted — no child runs without a ledger entry. |
| 6 | **Marginal-utility reason** | A short justification of why this expansion is expected to improve the outcome relative to terminating the current wave. | Block; emit Decision log `spawn_denied: no_marginal_utility`. |
...
Evidence sufficiency (check 4) is deterministic too — it asks whether evidence is
*reachable*, not whether it is *correct*.
...
| `correlation_id` | Optional. The correlation that ties this spawn to the originating user-facing request. Propagated to all child logs and Knowledge Layer reads. See [knowledge-layer.md §8](knowledge-layer.md). |
| `evidence_fetch_budget` | The portion of the parent's `evidence_fetch_budget` (`max_reads`, `max_refetches`, `max_stale_acceptance`) allocated to this child. See [governance.md §8](governance.md). |
...
| **Evidence shortfall** | Spawn-gate check 4 fails: the proposed child would act on or surface a claim with no reachable `claim_evidence_map_ref` path through the Knowledge Layer. The supervisor escalates rather than approving an evidence-free spawn. See [knowledge-layer.md §5](knowledge-layer.md). |
